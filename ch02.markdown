...
Les méthodes d'instance sur un des objets peuvent accéder aux variables d'intance et `this`.
La méthode 'distanceA()' dans l'example suivant est un exemple de méthode d'instance.
...
variable d'instance a un getter implicite, plus un setter le cas échéant.
...
class Rectangle {
  ...
  // Définie deux propriétés calculées : droite and bas.
...
getter est explicitement défini. Pour éviter des effets imprévus, l'opérateur appelle le 
getter exactement une fois, sauvant la valeur dans une variable temporaire.
</aside>

#### Méthods abstraites {#abstract-methods}
{:.no_toc}

Les méthodes d'instance, getters et setters peuvent être abstrait, définissant une
...
abstract class Actif {
  ...
  void faireQuelqueChose(); // Défine une méthode abstraite.
}

class ActifEfficace extends Actif {
  void faireQuelqueChose() {
    // ... Founit une implémentation, donc la méthode ici n'est pas abstraite...
  }
}
{% endhighlight %}
...
Si vous surcharger `==`, vous devriez surcharger le getter `hashCode` de Object.
