...
class Personne {
  final _nom;          // Dans l'interface, mais visible uniquement dans cette librairie,
  Personne(this._nom);   // Pas dans l'interface étant donné qu'il s'agit d'un constructeur.
  String saluer(qui) => 'Salut, $qui. Je suis $_nom.'; // Dans l'interface.
}

// Une implémentation de l'interface Personne.
class Imposteur implements Personne {
  final _nom = "";      // Nous avons besoin de la définir, mais ne l'utilisons pas.
...
{% highlight dart %}
class A {
  // A moins que vous ne surchargiez noSuchMethod, utiliser un membre inexistant
  // se terminera par une NoSuchMethodError.
  void noSuchMethod(Invocation mirror) {
    print('Vous avez essayer d'utiliser un membre inexistant : ${mirror.memberName}');
  }
}
{% endhighlight %}
...
<!-- ch02/ballgame.dart -->
{% highlight dart %}
library jeudeballe;   // Déclare qu'il s'agit d'une bibliothèque nommée jeudeballe.

import 'dart:html'; // Cette application utilise la bibliothèque HTML.
// ... Le code va ici ...
{% endhighlight %}
