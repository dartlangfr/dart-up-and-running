...
#### Paramètres nommés optionnels {#optional-named-parameters}
{:.no_toc}

Lors de l'appel à une fonction, vous pouvez spécifier un paramètre nommé par
...
#### Parmètres optionnels positionnés {#optional-positional-parameters}
{:.no_toc}
...
{% highlight dart %}
String dire(String de, String message, [String moyen]) {
  var resultat = '$de dit $message';
  if (moyen != null) {
    resultat = '$resultat avec un $moyen';
  }
  return resultat;
}
{% endhighlight %}
...
<!-- ch02/optional_positional_parameters.dart -->
{% highlight dart %}
assert(say('Bob', 'Salut') == 'Bob dit Salut');
{% endhighlight %}
...
<!-- ch02/optional_positional_parameters.dart -->
{% highlight dart %}
assert(say('Bob', 'Salut', 'signal de fumée') ==
...
String dire(String de, String message,
    [String moyen = 'pigeon voyageur', String humeur]) {
  var resultat = '$de dit $message';
  if (moyen != null) {
    resultat = '$resultat avec un $moyen';
  }
...
Toutes applications doit avoir une fonction de premier niveau `main()` qui
...
d'effectuer des opérations multiples sur le un même objet. Vous en saurez plus
...
environnantes. Dans l'exemple suivant, `makeAdder()` capture la variable `addBy`.
Partout où la fonction retounée est utilisée, il rapelle `addBy`.

<!-- ch02/function_closure.dart -->
{% highlight dart %}
/// Retourne une fonction qui ajoute [ajouterPar] 
/// à l'argument de la fonction.
Function makeAdder(num ajouterPar) {
...
Voici un exemple de test de fonctions de premier niveau, de méthodes
statiques, et d'égalité d'instances de méthodes.
...
main() {
  ...
  // Comparaison d'instance de méthodes.
...
l'instruction `return nul;` est implicitement ajoutée au corps de la fonction.
