...
## Librairies et visibilité {#libraries-and-visibility}

Les directives `import`, `part`, et `library` peuvent vous aider à créer
une base de code modulaire et partageable. Les libraries fournissent non seulement
des APIs, mais sont une unité de cloisonement : les identifiants qui commencent
par un underscore (\_) ne sont visibles qu'au sein de la librairie.
*Toute application Dart est une librairie*, même si elle n'utilise pas une directive library.

Les librairies peuvent être distribuées comme packages.
Voir [Pub Package and Asset Manager](/tools/pub/)
pour davantage d'informations sur pub, un gestionnaire de package inclu dans le SDK.


### Utiliser les librairies {#using-libraries}
{:.no_toc}

Utilisez `import` pour spécifier comment un espace de nommage d'une librairie est utilisé
dans le cadre d'une autre librairie.

Par exemple, les applications Web Dart utilisent généralement la librairie
...
Le seul argument nécessaire à `import` est une URI spécifiant la librairie.
Pour les librairies natives, les URI ont le préfixe spécial `dart:`.
Pour les autres librairies, vous pouvez utilisez un chemin de votre système
de fichier ou le préfixe `package:`. Le préfixe `package:` désigne les librairies 
fournies par un gestionnaire de package tel que l'outil pub. Par exemple :
...
#### Spécifier un préfixe de librairie {#specifying-a-library-prefix}
{:.no_toc}

Si vous importez deux librairies qui ont des identifiants en conflit, vous
avez la possibilité de spécifier un préfixe pour l'une ou les deux de ces librairies.
Par exemple, si librairie1 et librairie2 possèdent toutes deux une classe Element, 
...
var element2 = new lib2.Element(); // Utilise Element de lib2.
{% endprettify %}

#### Importer une partie d'une librairie {#importing-only-part-of-a-library}
{:.no_toc}

Si vous ne désirez utiliser qu'une partie d'une librairie, vous avez la possibilité
de choisir quoi importer de celle-ci. Par exemple :

<!-- ch02/libraries/library_partil.dart, lib1, lib2 -->
{% prettify dart %}
import 'package:lib1/lib1.dart' show foo; // Importe uniquement foo.
import 'package:lib2/lib2.dart' hide foo; // Importe tout SAUF foo.
{% endprettify %}


### Créer une librairie {#implementing-libraries}
{:.no_toc}

Utilisez `library` pour nommer une librairie, et `part` pour spécifier des fichiers additionnels
dans la librairie.
...
#### Déclarer une librairie {#declaring-a-library}
{:.no_toc}

Utilisez <code><em>l'identifiant</em> library</code>
pour spécifier le nom de la librairie courante :

<!-- ch02/ballgame.dart -->
{% prettify dart %}
library jeudeballe;   // Déclare qu'il s'agit d'une librairie nommée jeudeballe.

import 'dart:html'; // Cette application utilise la librairie HTML.
// ... Le code va ici ...
{% endprettify %}

#### Associer un fichier à une librairie {#associating-a-file-with-a-library}
{:.no_toc}
...
où *identifiant* est le nom de la librairie. L'exemple suivant
utilise `part` et `part of` pour implémenter une librairie de trois fichiers.

Le premier fichier `jeudeballe.dart`, déclare la librairie jeudeballe, importe 
les librairies qui lui sont nécessaires, et indique que `balle.dart` et `util.dart`
font partie de la librairie :
...
Le deuxième fichier, `balle.dart`, implémente une partie de la librairie jeudeballe :
...
Le troisième fichier `util.dart`, implémente le reste de la librairie jeudeballe :
...
#### Ré-exporter des librairies {#re-exporting-libraries}
{:.no_toc}

Vous pouvez combiner et repackager des librairies en ré-exportant tout ou partie de celles-ci.
Par exemple, vous pouvez avec une énorme librairie que vous implémentez sous forme de plus petites librairies.
Ou alors vous pouvez créer un librairie qui fournit un sous ensemble de méthodes d'une autre librairie.
