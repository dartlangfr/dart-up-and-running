...
|--------------------------|------------------------------------------------|
| postfix unaire           | <code><em>expr</em>++</code>    <code><em>expr</em>--</code>    `()`    `[]`    `.`             |
| prefix unaire            | <code>-<em>expr</em></code>    <code>!<em>expr</em></code>    <code>~<em>expr</em></code>    <code>++<em>expr</em></code>    <code>--<em>expr</em></code>   |
| multificatif           | `*`    `/`    `%`    `~/`                      |
| additif                 | `+`    `-`                                     |
| décalage                    | `<<`    `>>`                                   |
| ET binaire              | `&`                                            |
| OU exclusif binaire              | `^`                                            |
| OU binaire               | `|`                                            |
| comparatif&nbsp;et&nbsp;test de&nbsp;type | `>=`    `>`    `<=`    `<`    `as`    `is`    `is!`      |
| égalité                 | `==`    `!=`                                   |
| ET logique              | `&&`                                           |
| OR logique               | `||`                                           |
...
### Opérateur d'assignation {#assignment-operators}
{:.no_toc}
...
Voici comment fonctionnent les opérateur d'assignation :

|-----------+----------------------+------------------------|
|           | Assignation composée | Expression équivalente |
|-----------+----------------------+------------------------|
|**Pour une opérateur <em>op</em>:** | <code>a <em>op</em>= b</code> | <code>a = a <em>op</em> b</code>
|**Exemple:**                     |`a += b`                       | `a = a + b`
{:.table}
...
Vous pouvez inverser ou combiner des expressions booléenes en utilisant un opérateur
logique.

|-----------------------------+-------------------------------------------|
| Operateur                   | Signification                             |
|-----------------------------+-------------------------------------------|
| <code>!<em>expr</em></code> | Inverse l'expression qui le suite (change faux en vrai, et vice versa)
| `||`                        | OU logique
| `&&`                        | ET logique
{:.table .table-striped}
...
vous utiliserez ces opérateurs binaire et de décalage de bits avec des entiers.
...
| `|`                         | OU
| `^`                         | OU Exclusid
| <code>~<em>expr</em></code> | Complément binaire à 1 (0s become 1s; 1s become 0s)
| `<<`                        | Décalage à gauche
| `>>`                        | Décalage à droite
{:.table .table-striped}
...
|-----------+-------------------------------------------|
| Operateur | Nom              |              Signification                                   |
|-----------+-------------------------------------------|
| `()`     | Applique une fonction | Représente un appel de fonction
| `[]`     | Accède une liste          | Réfère à la valeur à l'index spécifié dans la liste
| <code><em>expr1</em> ? <em>expr2</em> : <em>expr3</em> | Conditionnel | Si _expr1_ est vraie, exécute _expr2_; sinon, exécute _expr3_
| `.`      | Accède un membre        | Réfère à une propriété d'une expression; exemple: `foo.bar` selectionne la propriété `bar` de l'expression `foo`
...
Attraper, ou capturer une exception interromp la propagation de l'exception. Attraper une exception vous offre la possibilité de la trairer :
...
Utiliez `on` quand vous avez besoin de spécifier le type de l'exception. Utilisez `catch` lorque traitement de l'exception a besoin de l'objet exception.
...
Au lieu d'utiliser les threads, tout code Dart tourne au sein d'un d'*isolates*.
...
Un commentaire d'une ligne comment par `//`. Tout entre `//` et la
...
main() {
  ...
  print('Bienvenue à ma ferme de Llama !');
}
{% endprettify %}
...
Une commentaire de plusieurs lignes commence par `/*` et se termine par `*/`.
Tout entre `/*` et `*/` est ignoré par le compilareur Dart (a moins que le commentaire de soit un commentaire de documentation; voir la section suivante). Les commentaires de plusieurs lignes peuvent s'imbiquer.
