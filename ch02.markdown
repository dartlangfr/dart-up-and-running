...
## Un programme Dart basique {#un-programme-dart-basique}
...
:   Utilisez // pour indiquer que le reste de la ligne est un commentaire. 
    Vous pouvez également utiliser /\* ... \*/. Pour plus de détails, regardez
    [Commentaires](#commentaires).

`num`

:   Un type. Quelques un des autres types sont String, int, and bool.
...
:   L’interpolation de chaine: L’inclusion d’une variable ou d’une expression à l’intérieur d’une chaine de caractère. 
    Pour plus d’informations, Regardez 
    [Strings](#strings).

`main()`

:   La fonction spéciale, requise et depremier niveau où l’application s’éxécute. 
    Pour plus d’information, Regardez 
    [The main() function](#the-main-function).

`var`

:   Une façon de déclarer une variable sans avoir à spécifier son type.

<aside class="alert alert-info" markdown="1">
**Note:**
Notre code suite la convention décrite dans le [Dart Style
Guide.](/articles/style-guide/). 
Par exemple, nous utilisons des indentations à deux espaces.
</aside>


## Concepts important {#important-concepts}

Pendant que vous apprenez le langage Dart, gardez ces faits et concepts 
en mémoire :
...
-   Le typage statique (comme `num` dans l'exemple précédent) clarifie 
    votre intention et active la vérification des types par les outils, 
    mais cela reste optionnel. (Pendant que vous debuggez votre code, 
    vous pourrez remarquer que les variables dont le type n'a pas
...
-   Dart analyse tout votre code avant de le lancer. Vous pouvez fournir
    ...
    constantes à la compilation, pour détecter les erreurs ou encore pour
...
Le tableau suivant liste les mots clés du language Dart.
...
<sup>1</sup>  Les mots annotés **1** sont des *identifiants inclus*. 
...
Par exemple, si du code JavaScrpt a une variable nommée `factory`, 
il n’est pas nécéssaire de la renommer lorsque vous pmorter le code en Dart.
