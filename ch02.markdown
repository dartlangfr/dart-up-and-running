...
à une constante de compilation (telle que `deprecated`) ou un appel à un constructeur constant.
...
`@proxy`, voir [Extending a class](#extending-a-class).
...
Et voici un exemple d'utilisation de cette annocation @todo :
...
Une commentaire d'une ligne comment par `//`. Tout entre `//` et la
